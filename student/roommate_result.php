...
function matchRate($row,$post,$params=['tolerance','study','belongings','neatness','sleep']){
    $ratingPerParam = 100/count($params);
    $total = 0;
    foreach($params as $param){
        if($row[$param]==$post[$param]){
            $total+=$ratingPerParam;
        }
    }
    return $total;
}
      ...
      <div class="row">
        <div class="col-md-4">
          ...
          <div class="settings-block">
            ...
            <ul class="settings">
              <li class="setting"><a href="#" class="setting__link"><i class="fa fa-search"></i> Search For Roommate</a></li>
          ...
          <div class="col-md-5" style="position:relative;left: 80px; float: right;">
            <h3 style="color:#880000">Connect with Friends</h3><br/>
        ...
        $select = mysqli_query($con, "SELECT `gender` FROM `students` WHERE `stu_id` ='".$_SESSION['stu_id']."'");
        ...
        $select = mysqli_query($con, "SELECT * FROM  `students` WHERE `tolerance`='$tolerance' OR `study`='$study' OR `belongings`='$belongings' OR `neatness`='$neatness' OR `sleep`= '$sleep' AND `clearance` = 1 AND `gender`='$gender' AND `stu_id` != '".$_SESSION['stu_id']."'");
        $num_rows = mysqli_num_rows($select);
        if ($num_rows == 0) {
          echo "<h4 style='margin-top:80px'>No Search Result Found!</h4>";
        }else{
        while ($row = mysqli_fetch_assoc($select)) {
            $match = matchRate($row,$_POST);
          # code...
         ?>
        <!-- Blog Post -->
        <div class="card" style="margin-top: 30px; text-align: center;">
          ...
          <div class="col-md-6">
            <h5 class="card-title"><?php echo $row['firstname'].' '.$row['lastname']; ?><br>
                <?php if($match < 50){ ?>
                <span style="color:red; font-size:28px"><?php echo $match.'% Match'; ?></span>
            <?php  }else{ ?>
                <span style="color:green; font-size:28px"><?php echo $match.'% Match'; ?></span>
               <?php } ?>
              </h5>
          </div>
          </div>
